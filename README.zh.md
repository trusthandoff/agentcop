...
- **信任链层（TrustChain Layer）** — 13 个模块的密码学信任链：SHA-256 链接的执行节点、Ed25519 签名认证、工具边界执行、来源追踪、上下文/内存污染检测、Agent 层级结构、跨运行时可移植性，以及 OTel/LangSmith/Datadog 导出。全部 10 个适配器已更新，新增可选信任参数。🔐 **CHAIN VERIFIED**
...
## 信任链层（TrustChain Layer）

`agentcop` v0.4.11 新增密码学信任链，对多 Agent 执行过程中的每个步骤进行验证。每个节点——工具调用、Agent 交接、RAG 查询、内存读取——均被哈希并与前一节点链接。链条断裂意味着某处发生了未经授权的变更。13 个模块，零新增强制依赖，当可选的 `cryptography` 包存在时使用 Ed25519 签名，否则退回仅哈希模式。
...
adapter = LangGraphSentinelAdapter(
    thread_id="run-abc",
    hierarchy=hierarchy,   # 未授权交接时抛出 PermissionError
)
```
...
| 模块 | 功能 |
|---|---|
| `agentcop.trust.chain` | SHA-256 链接的 `TrustChainBuilder`——添加节点、验证链条、导出 |
| `agentcop.trust.models` | 纯 dataclass 值对象：`TrustClaim`、`TrustChain`、`ExecutionNode` |
| `agentcop.trust.attestation` | 通过 `cryptography` 进行 Ed25519 签名；缺失时退回哈希模式 |
| `agentcop.trust.boundaries` | `ToolTrustBoundary`——声明并执行工具间数据流规则 |
| `agentcop.trust.provenance` | `ProvenanceTracker`——记录指令来源，检测伪造的 `user` 声明 |
| `agentcop.trust.lineage` | `ExecutionLineage`——有序步骤日志、差异对比、JSON/Mermaid/文本导出 |
| `agentcop.trust.context_guard` | `ContextGuard`——快照上下文哈希，检测注入模式的变更 |
| `agentcop.trust.rag_trust` | `RAGTrustLayer`——按文档的信任注册表，污染检测 |
| `agentcop.trust.memory_guard` | `MemoryGuard`——基于哈希的内存完整性，污染模式匹配 |
| `agentcop.trust.hierarchy` | `AgentHierarchy`——上下级委托图、否决权、法定投票数 |
| `agentcop.trust.interop` | `TrustInterop`——可移植的 `agentcop.trust.v1.*` 声明，OpenAI/Anthropic 格式 |
| `agentcop.trust.observability` | `TrustObserver`——OTel span、LangSmith run、Datadog trace、Prometheus |
| `agentcop.trust`（包） | 重新导出全部 28 个公开符号；`RAGPoisoningAlert` 和 `MemoryPoisoningAlert` 别名 |

### 适配器信任参数（v0.4.11+）

全部 10 个适配器现在支持可选信任参数：

```python
# Agent 适配器（LangGraph、AutoGen、CrewAI、Haystack、LlamaIndex、Semantic Kernel）
adapter = LangGraphSentinelAdapter(
    thread_id="run-abc",
    trust=TrustChainBuilder(agent_id="my-graph"),  # 记录每个已完成的节点
    attestor=NodeAttestor(private_key_pem=key),     # 签署每个声明（可选）
    hierarchy=AgentHierarchy(...),                  # 执行委托规则
    trust_interop=TrustInterop(),                   # 导出可移植声明
)

# 可观测性适配器（LangSmith、Langfuse、Datadog）
adapter = LangSmithSentinelAdapter(
    trust_observer=TrustObserver(),  # 运行完成时调用 record_verified_chain()
    hierarchy=AgentHierarchy(...),
)

# Moltbook
adapter = MoltbookSentinelAdapter(
    rag_trust=RAGTrustLayer(),       # 验证每条收到帖子的 submolt 来源
    trust_observer=TrustObserver(),
    hierarchy=AgentHierarchy(...),
)
```

所有参数默认为 `None`——现有代码无需修改。完整指南请参阅 [docs/guides/trust-chain.md](docs/guides/trust-chain.md)。
